...
βᴬ = scale_β * βᴵ
# Exposed rate
ηᵍ = epiparams_dict["ηᵍ"]
# Asymptomatic rate
αᵍ = epiparams_dict["αᵍ"]
# Infectious rate
μᵍ = epiparams_dict["μᵍ"]
...
# θᵍ = [0.00, 0.008, 0.047]
θᵍ = reduce(hcat, epiparams_dict["θᵍ"])'


# Hospitalization probability
# γᵍ = [0.0003, 0.003, 0.026]
γᵍ = reduce(hcat, [epiparams_dict["γᵍ"], epiparams_dict["γᵍ"] * epiparams_dict["risk_reduction_h"]])

# Fatality probability in ICU
# ωᵍ = [0.30, 0.30, 0.30]
ωᵍ = reduce(hcat, [epiparams_dict["ωᵍ"], epiparams_dict["ωᵍ"] * epiparams_dict["risk_reduction_d"]])

############################################

# Pre-deceased rate
ζᵍ = epiparams_dict["ζᵍ"]
# Pre-hospitalized in ICU rate
λᵍ = epiparams_dict["λᵍ"]
# Death rate in ICU
ψᵍ = epiparams_dict["ψᵍ"]
# ICU discharge rate
χᵍ = epiparams_dict["χᵍ"]
...
rᵥ = epiparams_dict["rᵥ"] # Relative risk reduction of the probability of infection
kᵥ = epiparams_dict["kᵥ"] # Relative risk reduction of the probability of transmission
